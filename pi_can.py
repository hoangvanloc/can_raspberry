...
def BSP_CAN_Scan_BroadCast(self):
    ...
    if retval < 0:
        return 0
    time.sleep(200 + devInfoMgt.broadInterval * 14)
    for j in range(0,14):
...
def BSP_CAN_StartHB_BroadCast(self):
    ...
    msg = can.Message(arbitration_id=_header.StdId,data=_temp,dlc=_txHeader.DLC,extended_id=False)    
    retval = bus.send(msg)
    return retval
def BSP_CAN_FreshLLD(self):
    ...
    for i in range(0,14):
        if evInfoMgt.dNodeOnline[i] != 0:
            ...
            msg = can.Message(is_remote_frame= _txHeader.RTR,arbitration_id=_txHeader.StdId,data=_temp,dlc=_txHeader.DLC,dlc=_txHeader.DLC,extended_id=False) 
            retval = bus.send(msg) 
            _timeOutNum = 10
